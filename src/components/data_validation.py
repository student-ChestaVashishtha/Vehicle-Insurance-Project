...
class DataValidation:
    ...
    def is_column_exist(self,df: DataFrame) -> bool:
        try:
            ...
            if len(missing_categorical_cols)>0:
                logging.info(f"Missing categorical column: {missing_categorical_cols}")
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            logging.info("Starting DataValidaion")
            ...
            if not status:
                validation_error_messsage += f"Columns are missing in test dataframe."
            else:
            ...
            data_validation_artifact=DataValidationArtifact(validation_status,message=validation_error_message,validation_report_file_path=self.data_validation_config.validation_report_file)
            report_dir=os.path.dirname(self.data_validation_config.validation_report_file)
            os.makedirs(report_dir,exist_ok=True)
            validation_report={
                "validation_status":validation_status,
                "error_message": validation_error_message
            }
            with open(self.data_validation_config.validation_report_file, "w") as report_file:
